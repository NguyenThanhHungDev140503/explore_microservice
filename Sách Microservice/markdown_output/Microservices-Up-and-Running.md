...
Hopefully this short overview gives you an idea of the scope of our model and exam‐
...
also explored how complexity and analysis paralysis present challenges to microser‐
vices adopters.
...
decisions. But technology alone won’t give you the value you need from a microser‐
vices  system.  Technology  is  important.  Good  technology  choices  make  it  easier  for
you  to  do  things  that  may  have  been  prohibitively  difficult.  At  its  best,  technology
opens doors and unlocks new opportunities. However, it’s useless on its own.
...
Cross-functional  teams  have  been  around  for  a  long  time,  dating
...
can reduce the amount of coordination that takes place between teams, our microser‐
...
end up with lots of microservices teams. Since we expect to have multiple microser‐
vices  teams  operating  in  our  system,  we  won’t  design  each  of  them  individually.
Instead,  we’ll  define  a  microservices  team  template  that  can  be  applied  to  any  new
teams we create. Think of this as creating a cookie cutter that we can use to “punch
out” some microservices teams later on when we need them. Or, if you have a pro‐
gramming background, you can think of this as defining a “class,” for which we’ll be
...
Over  time,  you  may  also  need  to  capture  the  interactions  that  must  take  place
...
Note that we’ve also drawn a small black arrow between the platform and microser‐
vices teams. This is to show that the platform team is implementing the x-as-a-service
...
So, open your Team Topology diagram and add a square (we’ve colored it red) near
...
fitting, given that the analyses performed with this methodology often prove to be the
...
goals. It should not be surprising. If we don’t even ask, “Who will be using this API?”
...
Harvard  Business  School  professor  Clayton  Christensen  names  this  observation  the
...
start  with  “as  a  <persona>,”  while  Job  Stories  disregard  the  persona  and  instead
...
The OAS describes RESTful APIs in a standard, tech stack–agnostic manner. It is gov‐
...
process such as SEED(S) before we jump into coding microservices. Overall, it saves
...
tional  APIs  once  we  start  writing  microservices,  or  do  the  smaller  APIs  (microser‐
vices)  replace  the  bigger  (conventional)  APIs?  We  have  often  seen  these  questions
cause a lot of confusion on teams trying to adopt microservices architecture.
...
Note  that  there  is  no  1:1  relationship  between  an  API  and  the
...
supposed to slice up our larger problem into smaller services to achieve the promised
...
sized  microservices.  Just  assuming  that  any  way  of  organizing  code  into  serverless
...
Cloud,  Amazon,  and  others,  eventually  found  themselves  having  a  lot  of  microser‐
vices! That, however, does not mean that these companies started with hundreds of
...
unified  domain  model  representing  the  entire  system.  Rather,  as  Evans  said  in  his
book:
...
Depending  on  the  type  of  coordination  and  coupling,  an  Upstream–Downstream
...
we make our entire company also learn and fall in love with it (because DDD is cer‐
...
The entire process is very interactive, requires the involvement of all participants, and
...
It is worth noting that it’s typically easier to split a service than to merge several serv‐
...
When  services  are  loosely  coupled,  a  change  to  one  service  should  not  result  in  a
...
We  will  start  exploring  techniques  for  avoiding  data  co-ownership  across  microser‐
vices  by  discussing  the  notion  of  microservice-embedded  data  in  the  following  sec‐
tions.
...
scenarios. There are use cases where the required functionality legitimately needs to
...
the datasets owned by multiple microservices. We will discuss how to implement dis‐
...
Microservices simplify building systems safely at scale. It is critical to clarify, however,
...
In  this  context,  by  “facts,”  Young  means  the  representative  values  of  event  occur‐
...
Let’s  look  at  an  example.  A  relational  data  model  describing  the  customer  manage‐
...
Design decisions of a technical nature are fairly straightforward when working with
...
tems and minimize data sharing? In such a case, we can prioritize consistency in the
...
containerized  infrastructure  and  deployment  pipelines  for  the  project.  This  step  is
...
and  CD  will  help  us  achieve  our  objectives.  We’ll  be  able  to  make  infrastructure
...
microservices  infrastructure.  But  the  first  step  is  to  install  some  tools  and  set  up  a
working environment.
...
code will be written in a special language called HCL that Terraform will be able to
...
our tooling and define a set of permissions for what our tools can do. We’ll use this
...
To add this policy, select “Attach existing policies directly” from the set of options at
...
If  everything  looks  OK  to  you,  click  the  Create  user  button.  You  should  now  see  a
...
Navigate to the AWS CLI download page and follow the instructions there to install
...
It’s possible to write code using GitHub’s browser-based text editor, but it’s not very
...
to  get  the  code.  So  the  first  step  we’ve  defined  is  to  check  out  our  Terraform  code
...
Because  this  is  a  book,  we’ve  identified  the  dependencies  you’ll
...
The last part of our YAML code uses HashiCorp’s Terraform setup action. As you can
...
steps  of  making  an  infrastructure  change.  We  created  and  edited  a  Terraform  file,
tested  and  ran  it  locally,  committed  it  to  the  repository,  and  tagged  it  to  kick  off  a
build and apply pipeline process. This sequence of steps is going to be our method for
...
Setting up the right infrastructure is vital to getting the most out of your microser‐
vices system. Microservices give us a nice way of breaking the parts of our application
...
cloud platform (AWS) rather than building a cloud-agnostic application that can be
...
If you want to learn about Kubernetes, a great introduction is pro‐
...
complex  and  require  special  attention.  To  facilitate  the  deployment  work  of  the
...
news is that not every principle and practice from traditional software development is
...
duction  environments  that  large  organizations  use  for  microser‐
vices.  But  space  constraints  prevent  us  from  giving  you  a
...
we  define  the  parameters  and  configuration  details  for  it  in  the  Terraform  syntax.
...
To meet those requirements, we’ll define a total of four subnets. Two of them will be
...
Add  the  Terraform  code  in  Example  7-5  to  implement  an  NAT  gateway  in  our
network.
...
If you need to debug your Terraform code, you can set the environ‐
...
Our infrastructure pipeline will apply Terraform changes, but before we kick it off we
...
that everything goes according to plan. If you don’t remember how to do that, you’ll
...
With all that in mind, let’s dive into the code that will make it come to life. We’ll be
...
automatically.  This  is  important  because  we  are  letting  EKS  automatically  provision
...
#EKS Configuration placeholder we put in the file earlier. You’ll also need to replace
...
format and validate the code we’ve written in the same way as we did for our other
modules:
...
Now, you can tag, commit, and push the Terraform file into your CI/CD pipeline just
...
You should get back a response indicating that there are no EKS clusters left in
your instance:
...
Robust  CI/CD  pipelines  are  crucial,  but  just  as  important  is  how  the  local  develop‐
...
ses of the same stack (e.g., Go, Java, etc.) should use a uniform approach and
...
we will dive into how to set up a solid containerized environment on major platforms,
...
The  JSON  file  you  will  be  editing  (multipassd-vm-instances.json)  should  look  some‐
thing like this:
...
-f  flag  when  we  try  to  execute  Docker  Compose.  Let’s  now  launch  MySQL  in  a
...
Following  the  SEED(S)  methodology,  next  we  describe  the  interactions  represented
...
Now that we have the actions and queries for our microservices, we can proceed with
...
A nice thing about the NodeBootstrap template is that it comes with full support for
...
This  will  create  a  simple  “Am  I  live?”  check  at  /ping  (known  as  a  liveness  probe  in
...
If  you  run  into  any  issues  while  modifying  the  code  yourself,  you  can  see  the  full
...
Redis  database.  This  is  implemented  in  accordance  with  the  popular  guidance  out‐
...
coordination  minimizations,  and  most  of  our  system  design  work  in  the  microser‐
vices style should indeed target the minimization of coordination needs. That said, at
...
changes to the code so that it will work in your AWS environment. To do that, we’ll
...
Terraform file and learn more about what it does. When you’re done, the top of your
...
When the environment is set up, run kubectl get svc --all-namespaces to con‐
...
There are plenty of registry hosting options available that support the Docker registry
...
In Chapter 9, we walked through the process of creating a flight information micro‐
...
You may have noticed that we aren’t adding any AWS account secrets to this reposi‐
...
remembering the process for repository creation, the GitHub Quickstart documenta‐
...
that work paid off as we ended up with the end-to-end deployment of the microser‐
vices architecture that we’ve been building throughout this book. We also got to reuse
...
We  didn’t  implement  observability  or  reporting  in  our  microser‐
vices architecture. That’s because we had limited space and wanted
to focus on some of the more foundational elements. But the good
...
working in a large organization. Coordination time is often a factor of organiza‐
...
worry about repeating the change in a live system. The actual colors of the environ‐
...
independent  authority  and  autonomy  (and  responsibility!)  over  infrastructure
changes.
...
This means we could spin up a new environment with our changes. After some test‐
...
But there are two areas where coordination costs are difficult to avoid in our up and
...
1. Deploy  the  new  version  of  the  microservice  as  a  canary,  alongside  the  existing
...
their  own  data.  This  decision  alone  should  help  constrain  the  scope  and  size  of  a
...
The root of our limitation is that we’re using a shared database instance to serve mul‐
...
• Many software engineers would like to have a solution that has a simple architec‐
...
that  genuine  poor  fit  is  not  always  the  cause  of  the  failure.  More  often  than  not,
...
early  stages  of  microservices  adoption  is  to  establish  a  “microser‐
vices police” that will strictly govern adherence to all of the micro‐
services principles and traits. Migrating to microservices is a long
